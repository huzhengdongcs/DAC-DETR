...
def evaluate(
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        targets_none = None
        outputs = model(samples, targets_none)

        balance_targets = copy.deepcopy(targets)
        balance_num = 2 
        # please note: In eval, we do not use the output of c-decoer, 
        # thus this value does not affect the result and is only used for the completeness of the code. You can set 1,2,3,...
        for target in balance_targets:
            target["boxes"] = target["boxes"].repeat(balance_num, 1)
            target["labels"] = target["labels"].repeat(balance_num)
        loss_dict = criterion(outputs, targets, balance_targets)
