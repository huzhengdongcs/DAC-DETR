...
class DeformableTransformer(nn.Module):
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, self_attn_mask=None, input_query_label=None, input_query_bbox=None):
        ...
        # decoder
        hs, inter_references,  hs_cdecoder,  inter_references_cdecoder, = self.decoder(
            tgt,
            reference_points,
            memory,
            spatial_shapes,
        ...
        inter_references_out_cdecoder = inter_references_cdecoder
...
class DeformableTransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        tgt_cdecoder = self.forward_ffn(tgt_cdecoder)
        return tgt, tgt_cdecoder
...
class DeformableTransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        intermediate_reference_points_cdecoder = []

        for lid, layer in enumerate(self.layers):
            if reference_points.shape[-1] == 4:
                reference_points_input = ( reference_points[:, :, None]  * torch.cat([src_valid_ratios, src_valid_ratios], -1)[:, None] )
                reference_points_input_cdecoder = ( reference_points_cdecoder[:, :, None]  * torch.cat([src_valid_ratios, src_valid_ratios], -1)[:, None] )
            else:
                assert reference_points.shape[-1] == 2
                reference_points_input = ( reference_points[:, :, None] * src_valid_ratios[:, None]  )
                reference_points_input_cdecoder = ( reference_points_cdecoder[:, :, None] * src_valid_ratios[:, None])

            if self.use_checkpoint:
                output, output_cdecoder = checkpoint.checkpoint(
                    layer,
                    output,
                    output_cdecoder,
                    query_pos,
                    reference_points_input,
                    reference_points_input_cdecoder,
                    src,
                    src_spatial_shapes,
                    src_level_start_index,
                    src_padding_mask,
            ...
            if self.bbox_embed is not None:
                ...
                tmp_cdecoder = self.bbox_embed[lid](output_cdecoder)

                if reference_points.shape[-1] == 4:
                    new_reference_points = tmp + inverse_sigmoid(reference_points)
                    new_reference_points = new_reference_points.sigmoid()

                    new_reference_points_cdecoder = tmp_cdecoder + inverse_sigmoid(reference_points_cdecoder)
                    new_reference_points_cdecoder = new_reference_points_cdecoder.sigmoid()
                    ...
                    new_reference_points_cdecoder = tmp_cdecoder
                    new_reference_points_cdecoder[..., :2] = tmp_cdecoder[..., :2] + inverse_sigmoid( reference_points_cdecoder )
                    new_reference_points_cdecoder = new_reference_points_cdecoder.sigmoid()
            ...
            if self.return_intermediate:  # intermediate_reference_points[5].size()  len(intermediate_reference_points)
                ...
                intermediate_reference_points_cdecoder.append(
                    new_reference_points_cdecoder
                    if self.look_forward_twice
                    else reference_points_cdecoder
                )


        if self.return_intermediate:
            return torch.stack(intermediate), torch.stack(intermediate_reference_points) , torch.stack(intermediate_cdecoder), torch.stack(intermediate_reference_points_cdecoder)
