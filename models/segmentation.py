...
def get_local_rank_o2m( quality, indices, ):
    #quality: one-dimension tensor 
    #indices: matching result
    bs = len(indices)
    device = quality.device
...
def IA_BCE_loss(src_logits,pos_idx_c, src_boxes, target_boxes, indices, avg_factor, alpha,gamma, w_prime=1, cdn=False, one2one=False):
